...
pub fn load_key(cfg: &Config) -> Option<String> {
    ...
    if let Ok(home) = std::env::var("HOME") {
        candidates.push(PathBuf::from(home).join(".config").join("codexbar-status").join("secrets").join("openrouter.json"));
    }
    for p in candidates {
        if !p.exists() {
            continue;
        ...
        let k = v.get("api_key").or_else(|| v.get("key")).and_then(|x| x.as_str()).unwrap_or("").trim();
        if !k.is_empty() {
...
fn num(o: &Value, key: &str) -> f64 {
    o.get(key).and_then(|x| x.as_f64().or_else(|| x.as_u64().map(|n| n as f64))).unwrap_or(0.0)
}
...
pub fn collect(cfg: &Config) -> OpenRouterStatus {
    let Some(key) = load_key(cfg) else {
        return OpenRouterStatus { needs_key: true, ..Default::default() };
    };
    ...
    let d = match http::get_json(&format!("{BASE}/credits"), &headers, 8) {
        ...
        Err(e) => {
            return OpenRouterStatus { needs_key: false, error: Some(e), ..Default::default() };
        }
    };
    let data = d.get("data").filter(|v| v.is_object()).cloned().unwrap_or_default();
    ...
    if let Ok(kd) = http::get_json(&format!("{BASE}/key"), &headers, 5) {
        let Some(k) = kd.get("data").filter(|v| v.is_object()) else { return st };
        ...
        if !(limit.is_finite() && limit > 0.0) {
            ...
            st.reset_window = k.get("limit_reset").and_then(|x| x.as_str()).map(|s| s.to_string());
            return st;
        }
